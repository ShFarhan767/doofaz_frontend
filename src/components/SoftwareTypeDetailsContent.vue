...
onMounted(async () => {
    ...
    try {
        ...
        const response = await axios.get(`${BASE_URL}softwareTypeDetails/${id}`);  // Use the id in the API call
        // Filter and sort the data based on frontendShowStatus and dataPosition
        softwareTypeData.value = response.data.data
        ...
        console.log('Data fetched and sorted successfully:', softwareTypeData.value);
    } catch (error) {
...
onMounted(async () => {
    const id = route.params.id;  // Get the id from route parameters
    try {
        const response = await axios.get(`${BASE_URL}softwareTypeInfo/${id}`);
...
<template>
    <div v-if="softwareTypeData && softwareTypeData.length">
        <!-- Iterate through the sorted softwareTypeData array -->
        <section v-for="(item, index) in softwareTypeData" :key="index" 
